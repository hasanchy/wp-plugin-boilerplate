...
class Auth extends Base {
	 ...
	 * @var string
	 */
	private $page_slug = 'wpplugbp_plugintest_auth';
	...
	public function init() {
		$this->page_title     = __( 'Google Auth', 'wpplugbp-plugin-test' );
		...
		$this->unique_id      = "wpplugbp_plugintest_auth_main_wrap-{$this->assets_version}";
	...
	public function prepare_assets() {
		...
		$handle       = 'wpplugbp_plugintest_authpage';
		$src          = WPPLUGBP_PLUGINTEST_ASSETS_URL . '/js/authsettingspage.min.js';
		$style_src    = WPPLUGBP_PLUGINTEST_ASSETS_URL . '/css/authsettingspage.min.css';
		...
		$this->page_scripts[ $handle ] = array(
			...
			'localize'  => array(
				...
				'restEndpointSave' => 'wpplugbp/v1/auth/auth-url',
				'returnUrl'        => '[Replace with the /wp-json/wpplugbp/v1/auth/confirm url]',
			),
		);
	}
	...
	protected function raw_script_data(): array {
		...
		if ( is_null( $script_data ) && file_exists( WPPLUGBP_PLUGINTEST_DIR . 'assets/js/authsettingspage.min.asset.php' ) ) {
			$script_data = include WPPLUGBP_PLUGINTEST_DIR . 'assets/js/authsettingspage.min.asset.php';
		}
		...
		if ( ! empty( $this->page_scripts ) ) {
			foreach ( $this->page_scripts as $handle => $page_script ) {
				...
				if ( ! empty( $page_script['localize'] ) ) {
					wp_localize_script( $handle, 'wpplugbpPluginTest', $page_script['localize'] );
				}
